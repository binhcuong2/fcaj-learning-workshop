...
1. [IAM Policies](5.6.1-IAM-Policies/): Xem xét và hiểu nguyên tắc Principle of Least Privilege được áp dụng cho mọi Lambda function trong SAM template.
2. [Quản lý API Keys](5.6.2-API-Keys-Management/): Lưu trữ và truy xuất an toàn các khóa nhạy cảm (SerpAPI, Gemini API Key) bằng AWS Systems Manager Parameter Store.
3. [Giám sát & Cảnh báo](5.6.3-Monitoring-Alarms/): Thiết lập CloudWatch Logs, Metrics và Alarms để giám sát lỗi Lambda, SQS Dead Letter Queue và độ trễ API Gateway.
4. [Chi phí & Ngân sách](5.6.4-Cost-Budget/): Cấu hình AWS Budgets để thiết lập ngưỡng chi tiêu và nhận thông báo email trước khi chi phí vượt quá giới hạn.
